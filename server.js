...
const GITHUB_OWNER = 'DEIN_GITHUB_USERNAME';
const GITHUB_REPO = 'spquiz';
const BRANCH = 'main';
...
async function loadJSON(filePath, fallbackPath){
  try{
    const res = await octokit.rest.repos.getContent({
      owner:GITHUB_OWNER, repo:GITHUB_REPO, path:filePath, ref:BRANCH
    });
    return JSON.parse(Buffer.from(res.data.content,'base64').toString());
  }catch{
    if(fs.existsSync(fallbackPath)) return JSON.parse(fs.readFileSync(fallbackPath));
...
async function saveJSON(filePath,fallbackPath,data){
  ...
  try{
    ...
    await octokit.rest.repos.createOrUpdateFileContents({
      owner:GITHUB_OWNER, repo:GITHUB_REPO, path:filePath, message:`Update ${filePath}`, content, sha, branch:BRANCH
    });
  }catch{
...
io.on('connection', socket=>{
  ...
  socket.on('getUsers', ()=> socket.emit('usersData', users) );
  ...
  socket.on('unlockBuzzers', ()=>{ currentGame.buzzersUnlocked=true; io.emit('buzzersUnlocked'); });
  socket.on('buzz', username=>{ if(!currentGame.activePlayer){ currentGame.activePlayer=username; io.emit('buzzUpdate',username); }});
  ...
  socket.on('answer', ({username,correct})=>{
    ...
    if(username===currentGame.activePlayer) updateScore(username,correct?points:-points/2);
    else updateScore(username,correct?points/2:-points/2);
